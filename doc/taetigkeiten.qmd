...
# wfpp_filmography_pauline_ids %>% 
#   filter(id_person %in% df_merged_all$id_person) %>% select(id_person, actors) %>% distinct() %>% View()
df_merged_all_mini <- df_merged_all %>% 
  filter(id %in% wfpp_filmography_pauline_ids$id)

## Detailansicht: WFFP Anzahl Rollen pro Frau

wfpp_filmography_occupations_counted <- wfpp_filmography_pauline_ids %>% 
...
dff_filmography_occupations_counted <- dff_film %>% 
  filter(uid_2 %in% df_merged_all_mini$filmportal_id) %>% 
...
dff_max_occupations <- df_merged_all_mini %>% 
  mutate(splitted = str_split(taetigkeiten, "[,]") %>% lengths(.)) %>% #View()
  arrange(desc(splitted)) %>% head(1) %>% pull(.)

dff_occupations <- df_merged_all_mini %>% 
...
Für jede der Sammlungen gibt es eine biografische Datei, die Tätigkeitsbeschreibungen enthält und auch eine filmografische Datei, die für jeden aufgelisteten Film die jeweilig erfassten Tätigkeiten nennt. Für Letztere kann dann über ein Ranking untersucht werden, wie gewöhnlich oder eben außergewöhnlich eine Tätigkeitsbeschreibung für eine Person ist.
...
Eine Auswertung ist hier mit vielen Unsicherheit behaftet, weil es nur so wenige Frauen sind. Für diese aber gilt, dass sie am ausführlichsten in den WFPP Biografien beschrieben sind. So vielfältige Tätigkeitsbeschreibungen wie dort, finden sich sonst nirgends. Die Biografien des DFF dagegen sind in der Beschreibung der Tätigkeiten im Vergleich sehr grob.  
...
df_all_occupations %>% 
  ...
  ggplot(., aes(x = nr_roles, y = type_2, fill = type_2)) +
  geom_col(alpha = 0.8) +
  # scale_size_area(max_size = 12) +
  scale_x_continuous(expand = c(0, NA)) +
  scale_color_manual(values = c("bio" = "cornflowerblue", "film" = "mediumseagreen", name = "")) +
  facet_col(vars(type)) +
  theme_b03_base + ffg_facets_polar + theme(legend.title = element_blank())
...
Jeder Kreis entspricht einer Quelle für die Tätigkeitsbeschreibung. 

- DFF-personenbezogenen Datensatz: innerster Kreis in grün
- WFPP-personenbezogenen Datensatz: erster blaue Kreis

Diese Informationen aus den biografischen Daten kommen immer nur einmal vor, deswegen werden die Kreise nicht skalliert.

- DFF-filmwerksbezogene Daten: grüner Kreis weiter außen
...
plot <- ggplot() +
  ...
  geom_point_interactive(data = dff_occupations, aes(x= plot_y, y = 2, size = counted, tooltip = paste0(role, ": ", counted)), color = "mediumseagreen") +
  geom_point_interactive(data = wfpp_occupations, aes(x= plot_y, y = 1, size = counted, tooltip = role), color = "cornflowerblue") +
  geom_point_interactive(data = dff_filmography_occupations_counted_vis, aes(x= plot_y, y = 3, size = counted, tooltip = paste0(role, ": ", counted)), color = "mediumseagreen") +
  geom_point_interactive(data = wfpp_filmography_occupations_counted_vis, aes(x= plot_y, y = 4, size = counted, tooltip = paste0(role, ": ", counted)), color = "cornflowerblue") +
