...
typo_sfb_mono <- "Monaco"

# typo_sfb_mono <- "Roboto Mono"
...
ffg_facets <- theme(
    strip.text = element_text(family = typo_sfb_mono, size = 11, hjust = 0),
    axis.text.y = element_blank(),
    panel.background = element_rect(fill = "#f2f2f2")
  )

ffg_facets_polar <- theme(
  strip.text = element_text(family = typo_sfb_mono, size = 10, hjust = 0),
  axis.text.y = element_blank(),
